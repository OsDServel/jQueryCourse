...
//code to toggle more-info visibility
	...
	$('#newsletter-checkbox').on('change', function(){
		if($(this).is(':checked')){
			...
			//you can also add in, "fadeIn()", "slideIn()", "show()"
		} else{
			// console.log('No')
			$('#newsletter-frequency').hide();
			//you can also add in,"fadeOut()", "slideOut()", "hide()"
		}
	})

	$('#newsletter-checkbox').trigger('change');
	//NOTE: this line is written so that when the box may be left checked, it may happen so its better to have jQuery know to trigger the box
	...
	$('#cart-form').on('submit', function(event){
		...
		let data= { form: $(this).serialize(),
			price: cart };
			//in this line we are making the data that we will pass by the checkbox, to be saved on a form, and called back as a serical number. so this line adds serial numbers to the data it stores in my 'data/cartSubmit.json' file.
